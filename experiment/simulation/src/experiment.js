...
function showCrossSectionFn(scope) {
  scope.CrossSection
    ? showHideCrossSection(scope, 1, 0.5, scope)
    : showHideCrossSection(scope, 0, 0, scope);
...
function gainSelectionFn(scope, arrow_check) {
  gainStTrigger = true;
  if (
    parseFloat(gain_value).toFixed(2) >= 0 &&
    parseFloat(gain_value).toFixed(2) < 93.0 &&
    arrow_check &&
    !button_clickFlag
  ) {
    gain_value += 1.86;
    ...
    getChild("needle").rotation = Number(
      parseFloat(gain_value + 135).toFixed(2),
    );
  } else if (
    parseFloat(gain_value).toFixed(2) > 0 &&
    parseFloat(gain_value).toFixed(2) <= 93.0 &&
...
function adjusteSelsectionFn(scope, arrow_check) {
  adjStTrigger = true;
  if (
    parseFloat(adjs_value).toFixed(2) >= 0 &&
    parseFloat(adjs_value).toFixed(2) < 93.0 &&
    arrow_check &&
    !button_clickFlag
  ) {
    gain_value += 1.86;
    ...
    getChild("needle").rotation = Number(
      parseFloat(adjs_value + 135).toFixed(2),
    );
    if (commonGainValue > commonAdjValue) {
      document.getElementById("error_section").style.display = "block";
    } else {
...
function vernierMove_rightArrow(scope, side_right) {
  ...
  vernierMove_rightInterval = setInterval(function () {
    if (getChild("smallScale").y > 166.44) {
      ...
      getChild("screw_Guage_Top").y = parseFloat(123 - vernier_clicks).toFixed(
        2,
      );
      smallScale_polygon.graphics
        .clear()
        .moveTo(103, parseFloat(200 - vernier_clicks).toFixed(2))
        .lineTo(130, parseFloat(200 - vernier_clicks).toFixed(2))
        .lineTo(131, parseFloat(225 - vernier_clicks).toFixed(2))
        .lineTo(129, parseFloat(238 - vernier_clicks).toFixed(2))
        .lineTo(105, parseFloat(238 - vernier_clicks).toFixed(2))
        .lineTo(102, parseFloat(225 - vernier_clicks).toFixed(2))
        .lineTo(103, parseFloat(200 - vernier_clicks).toFixed(2)).command;
      getChild("smallScale").y = parseFloat(200 - vernier_clicks * 0.9).toFixed(
        2,
      );
      getChild("smallScale").x = parseFloat(
        -4088 + vernier_clicks * 110,
      ).toFixed(2);
      getChild("zoom_Scale").x = parseFloat(
        -5644 + vernier_clicks_forZoom,
      ).toFixed(2);
      getChild("device_Front_two").y = parseFloat(
        -598 + vernier_clicks_device_Front,
      ).toFixed(2);
      //getChild("piston").y=parseFloat(0-vernier_clicks_forZoom).toFixed(2)
      getChild("piston").y = parseFloat(
        0 - vernier_clicks_device_Front,
      ).toFixed(2);
...
function mainCalculation(scope, side) {
  ...
  // Editing the velcocity calcualtion
  var velocityOfSound_liquid = Math.sqrt(
    1 /
      (comprasabilityArry[currentMediumSelected_number] *
        Math.pow(10, -10) *
        currentMediumSelected_Arry[currentMediumSelected_number]),
  ...
  console.log(
    "Density " + currentMediumSelected_Arry[currentMediumSelected_number],
  );
  console.log(
    "Compresability " + comprasabilityArry[currentMediumSelected_number],
  );
  ...
  var microammeterReading =
    commonAdjValue *
    Math.pow(Math.sin((1.57 / distance_millimeter) * commonVernierReading), 20);
  ...
  if (microammeterReading > commonGainValue) {
    getChild("needle").rotation = parseFloat(
      135 + microammeterReading * (0.938).toFixed(0),
    );
  } else if (microammeterReading < commonGainValue) {
    //getChild("needle").rotation=gain_NeedlePosition;
    getChild("needle").rotation = parseFloat(
      gain_NeedlePosition + microammeterReading * (0.938).toFixed(0),
    );
  }
  if (commonGainValue != 0 && microammeterReading < commonGainValue) {
    ...
    if (
      needleCount > 0 &&
      needleCount < 6 &&
      commonGainValue < commonAdjValue
    ) {
      if (getChild("needle").rotation <= gain_NeedlePosition) {
      } else {
    ...
    microammeterReading =
      commonAdjValue *
      Math.pow(
        Math.sin((1.57 / distance_millimeter) * commonVernierReading),
        20,
      );
...
function showHideResultFn(scope) {
  scope.showResult_mdl
    ? (document.getElementById("resultSection").style.display = "block")
    : (document.getElementById("resultSection").style.display = "none");
  scope.showResult_mdl
    ? (document.getElementById("resultSection_two").style.display = "block")
    : (document.getElementById("resultSection_two").style.display = "none");
  ultrasonic_Interferometer_stage.update();
}
